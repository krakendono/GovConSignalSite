...
type TakenOpportunitiesPageProps = {
...
export default async function TakenOpportunitiesPage({ searchParams }: TakenOpportunitiesPageProps) {
  ...
  return (
    <main className="min-h-screen bg-[linear-gradient(180deg,#f7f8f5,#eef2ff)] px-6 py-16">
      <section className="mx-auto max-w-5xl">
        ...
        <h1 className="mt-3 text-4xl font-semibold text-ink">Taken Opportunities</h1>
        <p className="mt-3 text-slate-700">Opportunities marked as successfully taken.</p>
        ...
        <section className="mt-8 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          {rows.length === 0 ? (
            <p className="text-sm text-slate-600">No taken opportunities yet.</p>
          ) : (
            <div className="space-y-4">
              {rows.map((row) => (
                <article key={row.opportunity_id} className="rounded-xl border border-slate-200 p-4">
                  ...
                  <p className="mt-2 text-xs text-slate-600">Why here: Taken opportunities are the ones you have successfully won.</p>
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    ...
                    <form action={setOpportunityStatus}>
                      ...
                      <input type="hidden" name="returnTo" value="/taken-opportunities" />
                      <button type="submit" className="rounded-lg border border-slate-300 bg-white px-3 py-1.5 text-xs font-medium text-slate-800 transition hover:bg-slate-50">Move back to active</button>
                    </form>
                  </div>
                </article>
              ))}
