...
type ActiveOpportunitiesPageProps = {
...
export default async function ActiveOpportunitiesPage({ searchParams }: ActiveOpportunitiesPageProps) {
  ...
  return (
    <main className="min-h-screen bg-[linear-gradient(180deg,#f7f8f5,#eef2ff)] px-6 py-16">
      <section className="mx-auto max-w-5xl">
        ...
        <h1 className="mt-3 text-4xl font-semibold text-ink">Active Opportunities (Attempting)</h1>
        <p className="mt-3 text-slate-700">Opportunities your team is actively pursuing.</p>
        ...
        <section className="mt-8 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          {rows.length === 0 ? (
            <p className="text-sm text-slate-600">No active opportunities yet. Mark one from the opportunities page.</p>
          ) : (
            <div className="space-y-4">
              {rows.map((row) => (
                <article key={row.opportunity_id} className="rounded-xl border border-slate-200 p-4">
                  ...
                  <p className="mt-2 text-xs text-slate-600">Why here: Active opportunities are the items you are currently pursuing.</p>
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    <form action={setOpportunityStatus}>
                      ...
                      <input type="hidden" name="returnTo" value="/active-opportunities" />
                    ...
                    {row.opportunities.notice_url ? (
                      <a href={row.opportunities.notice_url} target="_blank" rel="noreferrer" className="rounded-lg text-xs font-medium text-signal hover:underline">Open notice</a>
                    ) : null}
                  </div>
                </article>
              ))}
